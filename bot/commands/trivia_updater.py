...
from discord import Client, DMChannel, Message
...
class TriviaUpdater:
    ...
    async def handle_list_trivias(
        ...
        try:
            ...
            await message.channel.send(
                "I've sent you a DM to show you all trivias available!"
            )

            params: Dict[str, str] = {"username": username}
            trivias: List[Dict[str, Any]] = await self.api_client.get_user_trivias(
                params
            )
            ...
            trivia_list = "\n".join(
                f"{i+1}. {trivia['title']} "
                f"Difficulty: {trivia['difficulty']} - Theme: {trivia['theme']}"
    ...
    async def ask_continue_update(
        self, message: Message, is_error: bool = False
    ) -> bool:
        """Asks user if they want to continue updating and handles the response"""
        question = (
            "Would you like to try updating something else? (yes/no)"
            if is_error
            else "Would you like to update something else? (yes/no)"
        )
        ...
        try:
            response = await self.client.wait_for(
                "message", timeout=30.0, check=check_continue
            )
            ...
            await message.author.send(
                "⏰ No response received. Update process completed!"
            )
            return False
    ...
    async def handle_update_trivia(self, message: Message) -> None:
        ...
        if not self.game_state.can_start_process(user_id, ProcessType.UPDATE):
            await message.channel.send(
                "❌ You already have an active process. Use $cancel to cancel it first."
            )
            return

        # Start update process
        self.game_state.start_process(
            user_id, ProcessType.UPDATE, str(message.channel.id)
        )
        ...
        try:
            ...
            # Select trivia
            await message.author.send(
                "Which trivia would you like to update? (Enter the number)"
            )
            ...
            def check(m):
                return (
                    ...
                    and 1 <= int(m.content) <= len(trivias)
                )
            ...
            command_logger.info(
                f"Selected trivia for update: {selected_trivia['title']}"
            )
            ...
            response = await self.client.wait_for(
                "message", timeout=30.0, check=check_option
            )
            field_choice = int(response.content)

            # Handle basic fields
            field_mapping = {1: "title", 2: "difficulty", 3: "theme"}
            ...
            response = await self.client.wait_for(
                "message", timeout=60.0, check=check_value
            )
            ...
            if field_choice == 1:
                while True:
                    existing_trivias = await self.api_client.get_user_trivias(
                        {"username": message.author.name}
                    )
                    existing_titles = [
                        t["title"]
                        for t in existing_trivias
                        if t["id"] != selected_trivia["id"]
                    ]
                    command_logger.info(f"Existing titles: {existing_titles}")

                    if new_value in existing_titles:
                        command_logger.info(
                            f"Update cancelled: title '{new_value}' already exists"
                        )
                        await message.author.send(
                        ...
                        try:
                            ...
                            new_value = response.content
                            command_logger.info(
                                f"New title attempt received: {new_value}"
                            )
                        except TimeoutError:
            ...
            update_data = {
                field_mapping[field_choice]: int(new_value)
                if field_choice == 2
                else new_value
            }
            command_logger.info(
                f"Preparing to update trivia {selected_trivia['id']} "
            ...
            try:
                ...
                command_logger.info(
                    f"Successfully updated trivia {selected_trivia['id']}"
                )
                ...
                command_logger.error(
                    f"Error updating trivia {selected_trivia['id']}: {str(e)}"
                )
                await message.author.send(
                    "❌ Error updating the trivia. Please try again."
                )
    ...
    async def handle_theme_update(self, message, trivia_id):
        ...
        try:
            ...
            theme_list += (
                "\nEnter a number to select an existing theme, "
                "or type a new theme name to create one:"
            )
            ...
            try:
                ...
                if 0 <= theme_index < len(themes):
                    ...
                    await message.channel.send(
                        "❌ Invalid theme number. Using the provided "
                        "value as a new theme name."
                    )
    ...
    async def handle_update_field(self, message, trivia_id, field):
        ...
        try:
            ...
            command_logger.info(
                f"Preparing to update trivia {trivia_id} with data: {update_data}"
            )
